# ehs-gemba-report-sytatus

EHS Gemba Report Status — a shared internal tool for EHS teams to log, track, and view gemba walk findings and report statuses in real time, replacing spreadsheets and chat.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "238494282+marcuscw-crypto@users.noreply.github.com"
  git config user.name "marcuscw-crypto"
  ```
